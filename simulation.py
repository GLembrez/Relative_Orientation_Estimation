...
class Simulation():
    ...
    def compensate_dynamics(self) :
        ...
        self.a1_corrected[:,0] = self.a1[:,0] + np.cross(self.g1_d[:,0],self.d1) + np.cross(self.g1[:,0],np.cross(self.g1[:,0],self.d1))
        self.a2_corrected[:,0] = self.a2[:,0] - np.cross(self.g2_d[:,0],self.d2) + np.cross(self.g2[:,0],np.cross(self.g2[:,0],self.d2))
